...
if ($action === 'login' && $method === 'POST') {
    ...
    if (!empty($user['password_created_at']) && !empty($user['password_expiration_days'])) {
        try {
            $createdAt = new DateTimeImmutable($user['password_created_at']);
            $expiresAt = $createdAt->modify('+' . (int) $user['password_expiration_days'] . ' days');
            $passwordExpired = $expiresAt <= new DateTimeImmutable('today');
        } catch (Exception $e) {
            // Fecha mal formada (ej. '0000-00-00'): se ignora la expiración en vez de romper el login.
            $passwordExpired = false;
        }
    }
    ...
    $user['must_change_password'] = (int) $user['must_change_password'] === 1 || $passwordExpired ? 1 : 0;
